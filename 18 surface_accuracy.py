...
def main():
    ...
    # Shrink excess generated parts of the surface, that extend beyond its boundaries
    delta = 0.5
    ci = out.NewCellIterator()
    ci.InitTraversal()
    while not(ci.IsDoneWithTraversal()):
        pnts = ci.GetPoints()
        print(ci.GetCellId(), ci.GetCellType())
        mark = False
        for i in range(pnts.GetNumberOfPoints()):
            pnt = pnts.GetPoint(i)
            print(pnt)
            if (pnt[0]<0-delta) or (pnt[0]>4+delta): mark = True
            if (pnt[1]<-0.5-delta) or (pnt[1]>0+delta): mark = True
            if (pnt[2]<0-delta) or (pnt[2]>4+delta): mark = True
        if mark: 
            out.DeleteCell(ci.GetCellId())
            print(ci.GetCellId())
        ci.GoToNextCell()
